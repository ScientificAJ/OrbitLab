...
| State               | Meaning                                                                     | Expected product behavior                                                    |
| ------------------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `ready`             | Registered files exist and the actual SHA-256 matches the expected SHA-256. | The service may run inference if the input contract is valid.                |
| `unavailable`       | Required files, runtime, registry, or setup are missing.                    | The API/UI must show unavailable readiness, not a fake score.                |
| `checksum_mismatch` | Files exist but do not match the registered checksum.                       | The artifact must be rejected until refetched or deliberately re-registered. |
| `unsupported`       | The artifact exists but does not match the model service contract.          | The model surface should report the incompatibility clearly.                 |
...
## Release-Room Provenance

Public releases include model artifact evidence in the Science Provenance Release Room generated by:

```bash
python scripts/build_release_room.py --tag vX.Y.Z --clean
```

The release-room `model-artifact-checksums.json` records the registry path, registry checksum, every registered model entry, whether the path exists, the actual artifact checksum, and any mismatch/unavailable detail. This file is uploaded as a GitHub release asset for public releases.

Release-room model evidence is an audit record. It proves which artifacts were available and checksum-valid when the packet was generated. It does not prove that any specific target is a confirmed planet, and it does not replace the per-result ML evidence emitted by the API.
