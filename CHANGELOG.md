...
- SWEET sinusoid vetting adopts the DR25 Robovetter amplitude-vs-depth gate (Thompson et al. 2018): a sinusoid warns only when it is both significant and at least 50% of the transit depth. Bare sigma fired on essentially every real star (live: WASP-126 b flagged over a 56 ppm sine under a 6024 ppm transit); significant-but-tiny variability now surfaces as a non-blocking `stellar_variability_note` info flag.
- Nigraha depth features (`Depth`/`DepthEven`/`DepthOdd`) are fed in the upstream TLS convention (mean in-transit flux, `1 - depth`) instead of the fractional depth, which had put every target outside the trained 0.27-1.0 range and monotonically punished deep transits (clean synthetic hot Jupiter: 0.07 -> >0.99 after the fix; noise and sinusoid controls still score ~0). Verified against the pinned upstream training catalog.
- Difference-image centroiding is now a DV-style point-source fit (elliptical Gaussian + background, noise-weighted, covariance uncertainties) on the out-of-transit and difference images, localizing which star the transit comes from; a blended-neighbor eclipse is fitted onto the neighbor at >3 sigma where image moments smeared it toward the bright target. Moments remain as an honest fallback with `centroid_method` provenance.
- Paper-grade TLS SDE gating is calibrated per population (mission x cadence x baseline x red-noise bin) against empirical null distributions with a GEV tail fit (`sde_calibration.py` + `scripts/calibrate_sde_thresholds.py`), so the SDE bar encodes a constant false-alarm probability instead of one number for every star; the published 7.0 stays as a hard floor and uncovered bins honestly report `uncalibrated_floor`.
